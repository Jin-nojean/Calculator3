...
# 🚢 FuelEU Maritime 계산기
    ...
    # 🌿 Surplus 상태 - 화석연료 풀링 가능량 계산 (Δ1 + Δ2)
        if result["avg_ghg_intensity"] < result["standard_now"]:
            ...
            st.dataframe(pd.DataFrame(pooling_table))
            ...
            # 🔺 Deficit 상태 - 친환경 연료 필요량 
                ...
                st.dataframe(pd.DataFrame(green_table))
        ...
        if "avg_ghg_intensity" in result and "total_energy" in result:
            ...
            # 스텝 함수
            plt.step(years, standard_values, where='post', color="blue", linewidth=2)

            # 스텝 함수 레이블 (좌측 하단 고정)
            plt.text(
            min(years),                          # X축 시작점 (좌측)
            min(standard_values) - 2,           # Y축 기준보다 약간 아래 (하단)
            "TARGET GHG Intensity",             # 레이블 텍스트
            color="blue",
            fontsize=9, bbox=dict(facecolor='white', edgecolor='blue', boxstyle='round,pad=0.3'),
            ha='left',
            va='top'
            )

            # 평균 GHG Intensity 빨간선
            plt.hlines(avg_ghg_intensity, 2025, 2050, colors="red", linestyles="--", linewidth=2)

            # 빨간선에 평균 GHG 텍스트 추가
            plt.text(
            max(years) - 3, avg_ghg_intensity + 7,
            f"ACTUAL GHG INTENSITY: {avg_ghg_intensity:.2f} gCO₂eq/MJ",
            color="red", fontsize=9, bbox=dict(facecolor='white', edgecolor='red', boxstyle='round,pad=0.3')
)
            for start, end, value in steps:
